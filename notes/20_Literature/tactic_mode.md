すばらしい整理です！項モードの「構造を作る」視点に対し、タクティク・モードは「目標（ゴール）を削る」視点になります。

ご要望に合わせて、タクティク・モードにおける**「意思決定の地図」**として書き換えました。
...
## 🛠️ 命題論理のタクティク・モード対応表 (Mathlib拡張版)

| 論理記号 | 導入則 (ゴールを分解する) | 除去則 (仮定を活用する) |
| --- | --- | --- |
| **ならば ()** | `intro h` / **`rintro`** | `apply f` または `have` |
| **かつ ()** | `constructor` / **`⟨h1, h2⟩`** | `cases h` / **`rcases h`** / **`obtain`** |
| **または ()** | `left` または `right` | `cases h` / **`rcases h`** |
| **同値 ()** | `constructor` | `rw [h]` / `apply h.mp` |
